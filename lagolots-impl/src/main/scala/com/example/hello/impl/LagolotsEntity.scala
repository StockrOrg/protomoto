...
/**
  * This is an event sourced entity. It has a state, [[LagolotsState]], which
  ...
  */
class LagolotsEntity extends PersistentEntity {

  override type Command = LagolotsCommand[_]
  override type Event = LagolotsEvent
  override type State = LagolotsState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: LagolotsState = LagolotsState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case LagolotsState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        LagolotsState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class LagolotsState(message: String, timestamp: String)

object LagolotsState {
  /**
    ...
    */
  implicit val format: Format[LagolotsState] = Json.format
}

/**
  * This interface defines all the events that the LagolotsEntity supports.
  */
sealed trait LagolotsEvent extends AggregateEvent[LagolotsEvent] {
  def aggregateTag = LagolotsEvent.Tag
}

object LagolotsEvent {
  val Tag = AggregateEventTag[LagolotsEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends LagolotsEvent
...
/**
  ...
  */
sealed trait LagolotsCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends LagolotsCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends LagolotsCommand[String]
...
/**
  ...
  * application loader.
  */
object LagolotsSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[LagolotsState]
  )
}
